<script setup lang='ts'>
    ...
    import {
        validFileTypesRecord, filesReducer,
        type FileUploader, type UploadResult, validFileTypes
    ...
    const data = reactive<FileUploader>({    
        ...
        onChange: (e: Event) => {
            ...
            if (data.invalidFiles.length) {
                ...
                }.\n\nThe following files are not one of the above file formats: ${
                    data.invalidFiles.map(f => `\n- ${f.name}`).join(" ")
                }`);
            }
        },
        removeSelectedFileAt: (index: number) => () => {
            data.selectedFiles = [...data.selectedFiles.slice(0, index), ...data.selectedFiles.slice(index + 1)];
        },
...
<template>
    <div class="button-wrapper">
        <button @click="data.makeNewFileInput">
            Select Files
        </button>
    
        <div v-if="data.selectedFiles.length">
            <Button @click="data.uploadFiles" :disabled="data.isUploadDisabled">
                Upload
            </Button>
        </div>
    </div>
    ...
    <div class="files-container">
        <div>
            ...
            <h5 v-if="data.failedFiles.length">The following files failed to upload (please try again):</h5>
