...
import {
  assignModifierToFieldsByPrefixes,
  makeDefaultConverter,
  toCardGroup,
  type BaseConverter,
...
const toRadioGroup: FieldGroupConverter =
  ...
  (fg: Fieldgroup): Node =>
    converter({
      ...
      ...fg,
      label: fg.children.length === 1 ? fg.label || fg.children?.[0].label : fg.label,
      warning: fg.children.length === 1 ? fg.children?.[0].warning : fg.warning,
      children: fg.children.reduce((acc: Node[], field: Node): Node[] => {
        ...
        return [
          ...acc,
          ...(options.map((option: SDKOptionType) => {
            return modifyField(
              {
                ...(field as Select),
 ...
 */
const regroupChildren =
  (childrenMap: Record<string, string>): FieldGroupConverter =>
  (converter = defaultConverter) =>
  (node: Fieldgroup): Node =>
    converter({
      ...node,
...
const toGridGroup: FieldGroupConverter =
  (converter = defaultConverter) =>
  (node: Fieldgroup): Node =>
    converter({
      ...node,
...
const toWindMitGroup: FieldGroupConverter =
  (converter = defaultConverter) =>
  (node: Fieldgroup): Node =>
    converter({
      ...node,
...
const modifierMap: Record<string, BaseConverter<Fieldgroup, Node>> = {
  ...
  ...assignModifierToFieldsByPrefixes(toCardGroup())(
    ['occupants-', ['adults', 'children', 'tenants', 'otherFamilies']],
    [
      'pets-n-',
      [
        'none',
