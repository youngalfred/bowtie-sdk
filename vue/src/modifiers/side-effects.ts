...
type AsyncRuleMap = Record<
  string,
  (field: InputNode, state: any, replaced: number[]) => GenericField['sideEffect']
>

const getAutoWith17DigitVin: AsyncRuleMap[string] =
  (field, store, [idxOfAuto = 0]) =>
  async (): Promise<void> => {
    await store.fetchAndFillAutoByVin(idxOfAuto, field)
  }

const overrideOptionsFor =
  (key: 'make' | 'model' | 'bodyType'): AsyncRuleMap[string] =>
  (field, store, [idxOfAuto = 0]) =>
  async isInitialRender => {
