...
const fieldsWithSideEffects = {
   ...
   */
  'auto.autos.n.year': overrideOptionsFor('make', ({ makes }) =>
    makes.map(({ description }) => ({ name: description, label: description })),
  ),
  'auto.autos.n.make': overrideOptionsFor('model', ({ models }) =>
    models.map(({ model }) => ({ name: model, label: model })),
  ),
  'auto.autos.n.model': overrideOptionsFor('bodyType', ({ bodyStyles }) =>
